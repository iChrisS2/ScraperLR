...
export async function sendTelegramNotification(product: ProductData, originalUrl?: string): Promise<boolean> {
  try {
    ...
    // Format the message with inline links
    const message = `🔥 **${product.nombre}** 💰 CNY ￥${product.precio.toFixed(2)} ≈ $${usdPrice} 🛒 [Kakobuy Link](${product.links.KakoBuy}) ${originalUrl ? `🔗 [Original Link](${originalUrl})` : ''}
